...
function updateHeaderBanner(customText = null) {
    const banner = document.getElementById('statusBanner');
    const activePlayer = players[activePlayerIndex];
    let displayText = customText || `${activePlayer.name}'s Turn to Roll!`;

    banner.innerHTML = `
        <div class="banner-text">${displayText}</div>
        <div class="banner-actions">
            <button class="btn btn-sm" onclick="startGame(gameMode)" style="background-color:#fec53b;">Restart Game</button>
            <button class="btn btn-sm" onclick="resetToHome()" style="background-color:#cc5a00;">End Game</button>
        </div>
    `;
}
...
function promptCurrentEvaluatingPlayer() {
    ...
    if (evaluationPhase === activePlayerIndex) {
        ...
        updateHeaderBanner(`[ROLLER] ${evaluatingPlayer.name}: You rolled ${currentRollTotal}! Place it or take a penalty.`);
    } else {
        // Passive Turn Rule Adjustment: Disable rolling, lock actions down to a Choice selection/Skip toggle
        document.getElementById('rollBtn').disabled = true; 
        document.getElementById('passBtn').innerText = "Skip / Do Nothing";
        updateHeaderBanner(`[PASSIVE] ${evaluatingPlayer.name}: You can optionally choose to use this roll value of ${currentRollTotal}.`);
    }
...
function endGame() {
    ...
    const banner = document.getElementById('statusBanner');
    banner.innerHTML = `<div class="banner-text" style="white-space: pre-line;">${winnerText}</div>`;
...
function executeAiEvaluation() {
    ...
    if (bestMove && (evaluationPhase === activePlayerIndex || highestUtility > 20)) {
        ...
        calculateScore(ai);
        updateHeaderBanner(`🤖 AI placed ${currentRollTotal} on the ${bestMove.color} row!`);
    } else {
        if (evaluationPhase === activePlayerIndex) {
            ai.missteps++;
            calculateScore(ai);
            updateHeaderBanner(`🤖 AI took a Misstep penalty!`);
        } else {
            updateHeaderBanner(`🤖 AI skipped using your roll.`);
        }
    }
